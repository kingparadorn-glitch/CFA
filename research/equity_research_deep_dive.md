...
**demo หลัก = P1 + P2** (report artifact ชัด, ROI วัดได้) · P3-P7 engine รองรับครบเช่นกัน (ดู pipeline ทีละอัน)
...
| `consensus_dev_fn` | `(tp, iaa_tp) → dev` | `|tp−iaa_tp|/iaa_tp` |
...
- **proxy company demo ยังไม่เคาะ** — candidate: MTC (มี oppday Q4 2025 deck, หุ้นการเงิน NIM ชัด, IAA consensus มี) — รอทีมยืนยัน
- IAA Consensus — มี API หรือต้อง scrape SETTRADE? ต้องเช็ค
- rating threshold (Buy>15%?) — ปรับตาม house convention จริง
- SEC Thailand analyst license — เนื้อละเอียดยังไม่ fetch เต็ม
- demo scope: ทำ P1+P2 จริง, P3-P7 โชว์เป็น breadth
